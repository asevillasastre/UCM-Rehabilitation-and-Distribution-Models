...
3   2500  2800  3510  3300
4   2500  2800  3510  2550
;
...
Ecasa(s,h) precio de casacion [€] la hora h en el modelo estoc
...
FDEM6 demanda esperada en el modelo 6.2
...
Efobj4.. obj =E= SUM((s,t,h)$(ord(s)=LUCI(s,h)), P(s,h)* (a(t)*EY1(s,t,h) + b(t)*EPT(s,t,h) + ca(t)*EArr(s,t,h) + cp(t)*Epar(s,t,h)));
Efdem(s,h)$(ord(s)=LUCI(s,h)).. DEMS(s,h) =E= sum(t, EPT(s,t,h)) + sum(g, EPH(s,g,h));
Efpmax(s,t,h)$(ord(s)=LUCI(s,h)).. EPT(s,t,h) =L= pmax(t)*EY1(s,t,h);
Efpmin(s,t,h)$(ord(s)=LUCI(s,h)).. pmin(t)*EY1(s,t,h) =L= EPT(s,t,h);
Efsubida(s,t,h,s2)$(ord(s)=LUCI(s,h) and (ord(s2)=LUCI(s,h-1) or ord(h)=1)).. EPT(s,t,h) - EPT(s,t,h-1) =L= rs(t);
Efbajada(t,h,s,s2)$(ord(s)=LUCI(s,h) and (ord(s2)=LUCI(s,h-1) or ord(h)=1)).. EPT(s,t,h-1) - EPT(s,t,h) =L= rb(t);
...
*Efbalance(s,g,h)$(ord(h)>1).. ERH(s,g,h) =E= sum(s2$(ord(s2)=LUCI(s,h-1)),ERH(s2,g,h-1)) - EPH(s,g,h) +apor(h,g);
Efbalance(s,g,h)$(ord(h)>1).. ERH(s,g,h) =E= sum(s2$(ord(s2)=LUCI(s,h-1)),ERH(s2,g,h-1)) - EPH(s,g,h) +apor(h,g);
EfbalanceI(s,g,h)$(ord(h)=1).. ERH(s,g,h) =E= RHI(g) - EPH(s,g,h) + apor(h,g) - EV(s,g,h);
*reservas(g,h,s,s_prima)$(ord(s)=existe_s(s,h) and(ord(s_prima)=existe_s(s,h-1) or ord(h)=1))..
* RH(g,h,s) =E= Rini(g)$(ord(h)=1) + RH(g,h-1,s_prima) - PH(g,h,s) + a(g,h);
*EfbalanceI(s,g,h)$(ord(s)=existe_s(s,h) and(ord(s_prima)=existe_s(s,h-1) or ord(h)=1)).. ERH(s,g,h) =E= RHI(g) - EPH(s,g,h) + apor(h,g) - EV(s,g,h);
Efrr(s,h)$(ord(s)=LUCI(s,h)).. sum(t, pmax(t)*EY1(s,t,h) - EPT(s,t,h)) =G=0.2*DEMS(s,h);
Efencendido(s,t,h)$(ord(s)=LUCI(s,h) and(ord(s)=LUCI(s,h-1) or ord(h)=1)).. EY1(s,t,h)-EY1(s,t,h-1) =E= EArr(s,t,h) - EPar(s,t,h);
*reservas(g,h,s,s_prima)$(ord(s)=existe_s(s,h) and(ord(s_prima)=existe_s(s,h-1)
* or ord(h)=1)).. RH(g,h,s) =E= Rini(g)$(ord(h)=1) + RH(g,h-1,s_prima) - PH(g,h,s) + a(g,h);

*$(ord(s)=existe_s(s,h) and(ord(s_prima)=existe_s(s,h-1) or ord(h)=1))
*$(ord(s)=LUCI(s,h) and(ord(s2)=LUCI(s,h-1) or ord(h)=1))
...
MODEL MODELO_6_DETERM /fobj4, fdem, fpmax, fpmin, ffluyente,
 freservamax, freservamin, fbalance, frr, fencendido/;
